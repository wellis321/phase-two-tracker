...
$id   = (int)($_GET['id'] ?? 0);
$stmt = $db->prepare('SELECT * FROM pm_risks_issues WHERE id = ?');
$stmt->execute([$id]);
$risk = $stmt->fetch();
if (!$risk) {
...
$f = [
    ...
    'owner_user_id' => $risk['owner_user_id'], 'raised_date' => $risk['raised_date'],
];
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete'])) {
        $db->prepare('DELETE FROM pm_risks_issues WHERE id = ?')->execute([$id]);
    ...
    if (!$errors) {
        $db->prepare(
            'UPDATE pm_risks_issues SET type=?, title=?, description=?, severity=?, status=?, owner_user_id=?, raised_date=? WHERE id=?'
        )->execute([
            $f['type'], $f['title'], $f['description'] ?: null, $f['severity'], $f['status'],
            $f['owner_user_id'] ?: null, $f['raised_date'], $id,
        ]);
...
$pageTitle  = 'Edit Risk / Issue';
$activePage = 'risks';
require __DIR__ . '/../includes/layout/header.php';
?>

<div class="page-header"><div><h1>Edit risk / issue</h1></div></div>
