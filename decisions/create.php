...
$f = ['title' => '', 'description' => '', 'needed_by_date' => '', 'decision_owner_user_id' => '', 'status' => 'open', 'outcome' => '', 'decided_date' => ''];
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $db->prepare(
            'INSERT INTO pm_decisions (title, description, needed_by_date, decision_owner_user_id, status, outcome, decided_date, created_by)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
        )->execute([
            $f['title'], $f['description'] ?: null, $f['needed_by_date'] ?: null,
            $f['decision_owner_user_id'] ?: null, $f['status'], $f['outcome'] ?: null,
            $f['decided_date'] ?: null, get_current_user_id(),
        ]);
...
<div class="page-header"><div><h1>Add decision</h1></div></div>
